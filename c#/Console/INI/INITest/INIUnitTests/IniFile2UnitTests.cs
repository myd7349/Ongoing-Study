namespace INIUnitTests
{
    ...
    [TestFixture]
    public class IniFile2UnitTests
    {
        [OneTimeSetUp]
        ...
        public void Test()
        {
            ...
            ini_.Write("server", "ip", "192.168.0.1");
            ini_.ReadString("server", "ip").Should().Equals("192.168.0.1");
            ini_.Write("server", "port", 80);
            ini_.ReadInt("server", "port").Should().Be(80);

            ini_.ReadString("server", "ip-not-existed", null).Should().BeNull();
            ...
            ini_.Write("server", "UseIPv6", false);
            ini_.ReadBoolean("server", "useipv6").Should().BeFalse();
            ini_.Write("server", "UseIPv6", true);
            ...
            ini_.Write("server", "UseIPv6", "no");
            ini_.Read("server", "UseIPv6", false, bool.TryParse).Should().BeFalse();

            ini_.Write("Session", "User", "Jerry");
            ini_.Write("Session", "Password", "admin");
            ini_.GetSections().Should().Equal("Server", "Session");

            var lastSeen = DateTime.Now;
            ini_.Write("Session", "LastSeen", DateTime.Now, dt => dt.ToString("o"));
            ...
            ini_.Write("math", "pi", 3.14);
            ini_.Write("math", "e", 2.71828);
