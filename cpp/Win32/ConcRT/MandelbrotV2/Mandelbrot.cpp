...
{
    concurrency::task_group DrawingTasks;
    concurrency::unbounded_buffer<BitmapPtr> MandelbrotImages;
};
...
std::unique_ptr<MandelbrotContext> mandelbrotContext;
...
void DrawMandelbrot(HWND hWnd, BitmapPtr bitmap);
                     ...
                     _In_ int       nCmdShow)
{
    ...
    mandelbrotContext = std::make_unique<MandelbrotContext>();
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
        {
            ...
            {
                BitmapPtr bitmap;
                if (concurrency::try_receive(mandelbrotContext->MandelbrotImages, bitmap))
                {
                    if (bitmap != nullptr)
                    {
                ...
                {
                    mandelbrotContext->DrawingTasks.run(
                        [hWnd, width, height]()
                        {
                            DrawMandelbrot(hWnd, std::make_shared<Gdiplus::Bitmap>(width, height));
                        }
                    );
                }
            }
    ...
    case WM_SIZE:
    {
        ...
        mandelbrotContext->DrawingTasks.cancel();

        // Wait for any existing tasks to finish.
        mandelbrotContext->DrawingTasks.wait();
        ...
        if (cx != 0 && cy != 0)
        {
            mandelbrotContext->DrawingTasks.run(
                [hWnd, cx, cy]()
                {
                    DrawMandelbrot(hWnd, std::make_shared<Gdiplus::Bitmap>(cx, cy));
                });
        }
    }
        break;
...
void DrawMandelbrot(HWND hWnd, BitmapPtr bitmap)
{
    ...
    concurrency::parallel_for(0u, height,
        [&](UINT row)
        {
    ...
    // Dismiss the scope guard because the bitmap has been properly unlocked.
    guard.dismiss();

    // Add the Bitmap object to image queue.
    concurrency::send(mandelbrotContext->MandelbrotImages, bitmap);
