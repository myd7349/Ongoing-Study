...
void GetCpuId(int (&cpuid)[4])
{
    ...
    cpuid[0] = static_cast<int>(s1);
    cpuid[1] = static_cast<int>(s2);
    cpuid[2] = static_cast<int>(s3);
    cpuid[3] = static_cast<int>(s4);
}


std::string CpuIdToString(const int (&cpuid)[4])
{
    std::ostringstream oss;
    oss << std::hex << std::uppercase << std::setfill('0');
    oss << std::setw(8) << static_cast<unsigned int>(cpuid[0])
        << '-'
        << std::setw(8) << static_cast<unsigned int>(cpuid[1])
        << '-'
        << std::setw(8) << static_cast<unsigned int>(cpuid[2])
        << '-'
        << std::setw(8) << static_cast<unsigned int>(cpuid[3]);
...
int main()
{
    int cpuid[4];
    GetCpuId(cpuid);
    std::cout << "CPU ID: " << CpuIdToString(cpuid) << std::endl;

    __cpuidex(cpuid, 0, 0);
    std::cout << "EAX == 0: " << CpuIdToString(cpuid) << std::endl;

    __cpuidex(cpuid, 1, 0);
    std::cout << "EAX == 1: " << CpuIdToString(cpuid) << std::endl;

    __cpuidex(cpuid, 3, 0);
    std::cout << "EAX == 3(Processor Serial Number): " << CpuIdToString(cpuid) << std::endl;
...
// [Reliable way of generating unique hardware ID](https://stackoverflow.com/questions/2842116/reliable-way-of-generating-unique-hardware-id)
// http://www.soft.tahionic.com/download-hdd_id/index.html
// [WIN32_Processor::Is ProcessorId Unique for all computers](https://stackoverflow.com/questions/1101772/win32-processoris-processorid-unique-for-all-computers)
