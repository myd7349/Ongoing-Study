...
char *windows_error_str(uint32_t retval)
{
    ...
    sprintf(err_string, "[#%08X] ", error_code);

    size = FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, error_code,
        MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), &err_string[strlen(err_string)],
        1024 - (DWORD)strlen(err_string), NULL);
    if (size == 0) {
        format_error = GetLastError();
        if (format_error)
            sprintf(err_string, "Windows error code %u (FormatMessage error code %u)",
                error_code, format_error);
        else
            sprintf(err_string, "Unknown error code %u", error_code);
    } else {
